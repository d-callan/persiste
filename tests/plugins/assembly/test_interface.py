...
def test_assembly_observation_adapter_populates_latent_states() -> None:
    ...
    obs_backend = AssemblyCountsModel(detection_efficiency=0.8, background_noise=0.02)
    ...
    data.observed_counts = {initial_state.stable_id: 1.0}

    ll = adapter.log_likelihood(data, constraint, graph=None)

    assert np.isfinite(ll)
    assert hasattr(data, "latent_states")
    assert data.latent_states
    assert adapter.last_latent_states is not None
    assert adapter.last_latent_states == data.latent_states


def test_fit_assembly_counts_returns_constraint_result() -> None:
    primitives = ["A", "B"]
    observed = {"A": 5, "B": 3}
    ...
    result = fit_assembly_counts(
        ...
        observation_config=AssemblyCountsConfig(detection_efficiency=0.85, background_noise=0.01),
