...
from ..constraints.utils import apply_gene_constraint_to_rates
...
class GeneContentModel(TreeLikelihoodModel):
    ...
    def log_likelihood(self, parameters: Dict[str, float]) -> float:
        ...
        constraint_params = {
            k: v for k, v in parameters.items() if k not in ("log_gain", "log_loss")
        }
        ...
        if self.use_rust and self.constraint.n_parameters() == 0:
            ...
            for fam_idx, fam_name in enumerate(self.data.family_names):
                base_params = RateParameters(
                    gain_rate=gain_rate,
                    loss_rate=loss_rate,
                    family_id=fam_name,
                )
                constrained = apply_gene_constraint_to_rates(
                    base_params,
                    self.constraint,
                    family_id=fam_name,
                )
                gain_rates[fam_idx] = constrained.gain_rate
                loss_rates[fam_idx] = constrained.loss_rate
            ...
            for fam_idx, fam_name in enumerate(self.data.family_names):
                ...
                transition_provider = SimpleBinaryTransitionProvider(
                    gain_rate=constrained.gain_rate,
                    loss_rate=constrained.loss_rate,
                )
...
class BaselineDiagnostics:
    ...
    def print_report(self, file=None):
        ...
        print(
            "  Mean transitions per branch: {0:.2f}".format(self.mean_transitions_per_branch),
            file=file,
        )
    ...
    def _print_baseline_sensitivity_warning(self, file=None):
        ...
        if self.gain_rate < 0.01 or self.loss_rate < 0.01:
            ...
            rate_ratio = max(self.gain_rate, self.loss_rate) / max(
                min(self.gain_rate, self.loss_rate), 1e-10
            )
            if rate_ratio > 100:
                print(
                    "  ⚠ Note: Highly imbalanced gain/loss rates (ratio: {0:.1f})".format(
                        rate_ratio
                    ),
                    file=file,
                )
...
class ComparisonResult:
    ...
    def print_report(self, file=None):
        ...
        constraint_params = {
            k: v for k, v in self.alt_result.parameters.items() if k not in ["log_gain", "log_loss"]
        }
        if constraint_params:
...
class GeneContentInference:
    ...
    def get_baseline_diagnostics(self, verbose: bool = True) -> BaselineDiagnostics:
        ...
        if self._baseline_diagnostics is None:
            ...
            # Mean branch length
            mean_branch_length = self.data.tree.branch_lengths[
                self.data.tree.branch_lengths > 0
            ].mean()
