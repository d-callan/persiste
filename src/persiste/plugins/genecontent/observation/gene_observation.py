...
class TipObservations:
    ...
    def get_family_presence(self, family_id: str) -> Dict[str, bool]:
        ...
        return {
            taxon: obs.is_present(family_id)
            for taxon, obs in self.observations.items()
        }
    ...
    def from_matrix(
        ...
        for i, taxon in enumerate(taxon_ids):
            ...
            observations[taxon] = GeneFamilyVector(
                presence=presence,
                taxon_id=taxon
            )
...
class GeneContentObservation:
    ...
    def get_tip_likelihood(
        ...
        if self.detection_prob >= 1.0:
            # Perfect observation
            return 1.0 if observed == state else 0.0
        else:
