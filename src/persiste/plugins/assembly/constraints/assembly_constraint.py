...
from persiste.plugins.assembly.features.assembly_features import (
    AssemblyFeatureExtractor, TransitionFeatures
)
...
class AssemblyConstraint(ConstraintModel):
    ...
    def __init__(
        ...
        feature_extractor: Optional[AssemblyFeatureExtractor] = None,
    ):
        """
        ...
        self.feature_extractor = feature_extractor if feature_extractor is not None else AssemblyFeatureExtractor()
    ...
    def constraint_contribution(
        ...
        # Compute weighted sum (Layer 2 - theory)
        C = 0.0
    ...
    @classmethod
    def assembly_theory(cls, reuse: float = 1.0, depth_penalty: float = -0.3) -> 'AssemblyConstraint':
    ...
    def pack(self, parameters: Optional[dict] = None) -> np.ndarray:
        """
    ...
    def unpack(self, vector: np.ndarray) -> dict:
        ...
        if len(vector) != len(keys):
            raise ValueError(
                f"Vector length {len(vector)} != expected {len(keys)}. "
                f"Expected features: {keys}"
            )
        
        return {k: float(vector[i]) for i, k in enumerate(keys)}
    
    def num_free_parameters(self, parameters: Optional[dict] = None) -> int:
        """
    ...
    def get_constrained_baseline(self, parameters: Optional[dict] = None):
        """
        ...
        if parameters is not None:
            ...
            old_params = self.feature_weights.copy()
