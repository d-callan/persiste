...
def analyze_scaling_curves(all_results: Dict[str, List[Dict]]) -> Dict:
    ...
    if identifiable_depths:
        ...
        print(f"✗ Not identifiable with any depth tested")
    ...
    if identifiable_samples:
        ...
        print(f"✗ Not identifiable with any sample size tested")
    ...
    if identifiable_times:
        ...
        print(f"✗ Not identifiable with any simulation time tested")
    ...
    for r in prim_results:
        ...
        print(f"{r['n_primitives']:>4d} {r['ll_range']:>8.1f} {r['runtime']:>8.1f} {r['n_states']:>8d} {marker:>4s}")
    ...
    for r in depth_results:
        marker = "✓" if r['identifiable'] else "✗"
        print(f"{r['max_depth']:>4d} {r['ll_range']:>8.1f} {r['runtime']:>8.1f} {r['n_states']:>8d} {marker:>4s}")
...
def generate_summary(all_results: Dict[str, List[Dict]], minimal_requirements: Dict):
    ...
    if max(prim_ranges) > 2 * min(prim_ranges):
        print(f"  • More primitives → stronger identifiability")
    ...
    if max(depth_ranges) > 2 * min(depth_ranges):
        print(f"  • Greater depth → stronger identifiability")
    ...
    if max(sample_ranges) > 2 * min(sample_ranges):
        print(f"  • More samples → stronger identifiability")
    ...
    if max(time_ranges) > 1.5 * min(time_ranges):
        print(f"  • Longer simulation → better equilibration")
