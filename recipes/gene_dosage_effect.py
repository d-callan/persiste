...
def gene_dosage_effect(
    ...
    retention_profiles = _run_genecontent_analysis(
        cn_data, family_names, taxon_names, tree, verbose
    )
    
    if verbose:
        print(f"\n✓ Classified {len(retention_profiles)} gene families")
        core_count = sum(1 for p in retention_profiles.values() if p.retention_regime == "core-like")
        accessory_count = sum(1 for p in retention_profiles.values() if p.retention_regime == "accessory-like")
    ...
    dosage_profiles = _run_copynumber_analysis(
        cn_data, family_names, taxon_names, tree, retention_profiles, verbose
    )
    ...
    integrated_patterns = _integrate_patterns(
        retention_profiles, dosage_profiles, verbose
    )
    
    # Create summary table
    summary_table = _create_summary_table(
        retention_profiles, dosage_profiles, integrated_patterns
    )
